...
class IntelligentBaselineEngine:
    ...
    def calculate_periodic_baseline(self, metric_key, period_type='hour'):
        ...
        for timestamp, value in time_series:
            if period_type == 'hour': period_key = timestamp.hour
            elif period_type == 'dow': period_key = timestamp.isoweekday() % 7
            elif period_type == 'hour_dow': period_key = (timestamp.isoweekday() % 7) * 24 + timestamp.hour
            else: period_key = 0
            period_values[period_key].append(value)
        periods = {}
    ...
    def detect_trend(self, metric_key, window_hours=24):
        ...
        return {'trend': trend, 'change_pct': round(change_pct, 2), 'confidence': round(min(abs(change_pct) / 20 * 100, 100), 2), 'avg_first_half': round(avg_first, 2), 'avg_second_half': round(avg_second, 2)}
    ...
    def get_current_period_baseline(self, metric_key, timestamp=None):
        ...
        if current_key in baseline.get('periods', {}):
            ...
            return {'mean': pd['mean'], 'std': pd['std'], 'p95': pd['p95'], 'p99': round(pd['p95'] * 1.1, 2), 'min': pd['min'], 'max': pd['max'], 'normal_range': [round(pd['mean'] - 2 * pd['std'], 2), round(pd['mean'] + 2 * pd['std'], 2)], 'period_info': {'type': 'hour_dow', 'key': current_key, 'dow': timestamp.isoweekday() % 7, 'hour': timestamp.hour, 'sample_count': pd['count']}}
