  ...
  tdsql: [
    ...
    },
    {
      ...
      metrics: [
        ...
        { key: 'tdsql_mode_display', label: '实例模式', format: 'text' },
        { key: 'tdsql_cluster_name', label: '集群名', format: 'text' },
        { key: 'tdsql_proxy_version', label: 'Proxy版本', format: 'text' },
        { key: 'tdsql_set_count', label: 'Set数量', format: 'number' },
        { key: 'tdsql_set_healthy_count', label: '健康Set数', format: 'number' },
        { key: 'tdsql_master_count', label: '主节点数', format: 'number' },
        { key: 'tdsql_slave_total', label: '备节点数', format: 'number' },
      ],
    },
    // === 表格型分类 ===
    {
      key: 'tdsql_sets',
      title: 'TDSQL Set 拓扑 (主备节点)',
      type: 'table',
      showWhen: (data) => data.tdsql_sets && data.tdsql_sets.length > 0,
      columns: [
        { key: 'set_name', title: 'Set名称' },
        { key: 'alias', title: '别名' },
        { key: 'health_display', title: '健康状态' },
        { key: 'master_ip', title: '主节点' },
        { key: 'slaves_display', title: '备节点' },
        { key: 'shardkey_range', title: 'Hash范围' },
        { key: 'partition_num', title: '分区数', format: 'number' },
        { key: 'idc_display', title: 'IDC' },
      ],
    },
    {
      key: 'tdsql_shard_tables',
      title: 'TDSQL 分片表配置 (shardkey)',
      type: 'table',
      showWhen: (data) => data.tdsql_shard_tables && data.tdsql_shard_tables.length > 0,
      columns: [
        { key: 'db_table', title: '表名' },
        { key: 'table_type', title: '表类型' },
        { key: 'shardkey', title: '分片键' },
        { key: 'auto_increment', title: '自增列' },
      ],
    },
    {
      key: 'tdsql_proxy_endpoints',
      title: 'TDSQL Proxy 网关节点',
      type: 'table',
      showWhen: (data) => data.tdsql_proxy_endpoints && data.tdsql_proxy_endpoints.length > 0,
      columns: [
        { key: 'endpoint', title: '网关地址' },
        { key: 'idc', title: 'IDC' },
      ],
    },
    {
    ...
    {
      key: 'tdsql_cluster_events',
      title: 'TDSQL 集群问题与提示',
      type: 'table',
      showWhen: (data) => data.tdsql_cluster_events && data.tdsql_cluster_events.length > 0,
      columns: [
        { key: 'level', title: '级别' },
        { key: 'description', title: '描述' },
      ],
