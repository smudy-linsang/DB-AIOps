...
## 附录 A：ProcessLease 迁移与施工设计

### A.1 数据模型

新增 `process_lease` 表，不修改或覆盖任何存量迁移。唯一键为 `(role, shard_key)`；`owner_id` 标识进程实例；`fencing_token` 为每次换主时单调递增的 64 位整数；`expires_at` 是数据库时钟语义下的租约截止时间；`heartbeat_at` 和 `metadata` 用于审计与可观测性。

### A.2 原子状态转换

抢占必须在 `transaction.atomic + select_for_update` 中完成。仅空租约、同 owner 或已过期租约可获得领导权；owner 变化时 token 加一。同 owner 续租必须同时匹配 owner/token 且旧租约尚未过期，暂停超过 TTL 的旧进程不得“复活”。释放使用 owner/token 条件更新，旧 leader 不能释放新 leader 的租约。

### A.3 运行时行为

- collector、sentinel、pipeline 启动时领取各自 `global` 分片，默认 TTL 30 秒、每 10 秒续租。
- 续租线程连续遇到元数据数据库故障时，仅可运行到已持有的 `expires_at`；到期或条件更新失败后设置失租事件，业务循环主动停止。
- 每个业务周期前调用 `assert_leader`。当前版本先实现全局单分片；后续按实例一致性 hash 扩成多个 shard 时沿用同一表和 token 语义。
- `--once` 同样必须持有租约，防止人工触发与常驻 leader 重叠。

### A.4 迁移与回退

迁移仅创建新表和唯一约束，不涉及存量数据清理。旧版本不读取该表，因此数据库迁移可向前兼容回退；应用回退时保留表。若租约存储不可用，生产进程 fail closed，不允许无锁继续；测试 SQLite 只验证顺序状态机，并将并发抢占验证放在 PostgreSQL integration 层。

## 附录 B：v2.5 施工落地状态与交付物

| 工作包 | 已落地施工项 | 尚需外部环境完成 |
|---|---|---|
| WP0 基线止血 | 4NF 保存、TSDB cursor、确定性 lint、expectedFailure 全转正 | 无 |
| WP1 执行面统一 | V1/V2 scope、真实阻塞图、统一 Playbook、审批、参数护栏、证据化 RCA | 行内工单和双人审批平台联调 |
| WP2 生产与供应链 | production fail-fast、完整哈希锁、SBOM、安全门禁、非 root 多阶段镜像、固定 digest | 行内镜像扫描、签名与准入 |
| WP3 可信诊断 | 无证据不成链、LLM 离线降级、案例蒸馏和 4NF、Webhook 出站限制 | 私有模型数据分级和完整 EvidenceRecord 演进 |
| WP4 HA/容量 | ProcessLease/fencing、角色心跳、live/readiness、性能回归脚本 | 500 实例、事件风暴、2 小时长稳和跨机房演练 |
| WP5 银行集成 | 接口与安全边界已设计 | IAM/MFA、CMDB、WORM、通知和五方 UAT |

实际部署施工顺序、TLS 入口、变量清单、备份、升级、Smoke、回退和故障处理以 `DEPLOYMENT.md` 为准。三份报告保留“基线失败事实”和“整改后复测事实”两套口径，禁止用最终绿灯覆盖问题发现过程。
