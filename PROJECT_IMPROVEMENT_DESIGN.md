...
>
> 2026-08-09 复议：Oracle 方言测试转正后曾考虑把它加为必需检查，
> **Owner 确认不做**（没有 PR 可拦、加了就堵直推、且多半是空转绿灯）。
> 详见附录 B.6.2。CI 在本仓库的定位是**事后信号**，不是合并门禁。
...
> **结局（2026-08-09，见 B.6）：`continue-on-error` 已摘掉，该 job 现在是
> 真门禁。** 四条用例在 `gvenzl/oracle-free:23` 上真跑且零 skip。
> 下面这一节保留当时的判断与理由，作为过程记录。

`test-dialect-oracle` **当时是信息性 job（`continue-on-error: true`），不阻断合并**。
...
跑通后的收尾动作：去掉 `continue-on-error`。
（原文还写了"并将其加入分支必需检查"，该项经 Owner 确认**不做**，理由见 B.6.2。）
...
加上这一步之后，第一次运行断言就挂了。我据此得出"Oracle 用例一直是全
skip"的结论 —— **这个结论是错的，断言下得太粗。**

随后补的两个探针 step（能否连上 Oracle / 能否读 `v$session`）**双双通过**，
测试步骤本身也跑了 7 秒并返回成功。真相是：4 条用例里只有 1 条 skip，
而且是**数据级 skip**，不是环境级 skip：

| skip 的性质 | 例子 | 含义 |
|---|---|---|
| 环境级 | `无法连接 Oracle` / `读不到 v$session` | 根本没验证，绿灯含金量为零 |
| 数据级 | `采样窗口内没有捕获到带 sql_id 的会话` | 连上了、跑了，只是没抓到目标数据 |

只看 `skipped=N` 把两者混为一谈，于是把"1 条数据级 skip"误读成"全 skip"。
**断言下得太粗，产出的不是信息而是假情报** —— 这跟它要解决的"绿灯有歧义"
是同一类错误，只是方向相反。断言已改为分别识别这两类。

顺带查出那条数据级 skip 其实是**用例自己写坏了**：它 `execute` + `fetchall`
之后才去采样，而语句一返回，那个会话立刻变回 INACTIVE、`sql_id` 被清掉，
于是几乎必然采不到 → 稳定 skip。用例把自己的设计缺陷伪装成了环境问题，
在 CI 上长期显示为一条无人过问的 `skipped=1`。已改为后台线程持续制造负载
＋ 15 秒内多次重试采样，采不到判**失败**而不是 skip（负载是自己造的，
采不到就是真问题）。

方法论层面的两条收获：

1. **当唯一可读的信道是红绿灯时，把要问的问题拆成若干个是非题，每题占
   一个 step。** step 的 conclusion 是 jobs API 直接可读的，而具体原因只写在
   job log 里，会被 Oracle 容器的启动日志淹掉。这条对任何"日志拿不到"的
   CI 排查都适用。
2. **skip 必须分级。** 一个长期挂在 CI 上、没人解释得清的 `skipped=N`，
   既可能是"环境不支持"，也可能是"用例写坏了"，而后者会一直伪装成前者。

### B.6.1 结果：Oracle job 转正为门禁

修完用例之后那一轮 CI，Oracle job 七步全绿，其中"确认用例真的跑了"这一步
通过，意味着 **`Ran 4 tests`、零 skip、零失败** —— 四条 Oracle 方言用例
（`call_timeout` / ASH 采样契约 / `sql_text` 补全 / 对象名缓存按实例隔离）
第一次在真实 Oracle 23 上被真正验证过。

据此执行 B.5 里写好的收尾动作：**摘掉 `continue-on-error`**，job 名去掉
"信息性"。它现在和 MySQL/PG 方言测试一样是门禁。

保留的护栏：那一步"确认用例真的跑了"**不能删**。它守住的是"绿灯 = 真的
验证过"这个性质；一旦哪天用例又退化成 skip，它会立刻把 job 判红。
若 `sql_text` 用例的 15 秒采样重试窗口在繁忙 runner 上出现偶发，
正确的回滚是把 `continue-on-error: true` 加回来，**而不是删掉断言** ——
删断言等于回到"绿灯含义不明"的老问题。

### B.6.2 「加入分支必需检查」——经确认不做

B.5 原本把"加入分支必需检查"列为收尾动作。**Owner 已确认不做**，
理由是它在本仓库的实际工作流下换不到任何拦截，却要付出真实代价：

1. **没有 PR 可拦。** 本仓库的实际工作流是直推 master、基本不开 PR
   （多智能体提交由 Owner 人工协调）。必需检查只在 PR 合并路径上生效。
2. **加了就不能直推。** 「允许直推」与「必需状态检查」在 GitHub 上互斥 ——
   直推的 commit 从没跑过 CI，必需检查永远停在 expected，推送被 GH013 拒绝。
   这正是 Owner 当初删掉 ruleset 的原因，再加一遍等于重演。
3. **就算加了也多半是空转的绿灯。** Oracle job 的触发条件是
   `push || contains(labels, 'test-dialect')`，没打标签的 PR 上它会被 skip，
   而 GitHub 把 skipped 的必需检查按**通过**计。门禁看着配上了，实际不拦任何东西
   —— 又回到"绿灯不代表验证过"，与本节全部工作背道而驰。
   （若将来真要做，前置动作是先去掉那个标签闸门，让它在所有 PR 上都跑。）

**因此本仓库的 CI 定位是"事后信号"而非"合并门禁"**：master 一旦挂红就是真红
（Oracle job 已摘掉 `continue-on-error`，且有"确认用例真的跑了"守住绿灯含义），
由 Owner 人工处置。这个定位是清醒的选择，不是遗留待办 ——
**请后续维护者不要把它当作"还没做完的事"再去配一遍。**
