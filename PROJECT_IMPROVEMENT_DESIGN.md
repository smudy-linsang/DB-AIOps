...
**加上这一步之后，第一次运行就把答案抖出来了：断言步骤挂了 —— Oracle 用例
一直是全 skip 的。**也就是说 B.5 里"连绿两轮"的那两个绿灯，含金量是零。
这正是 B.5 担心的事，只是当时还只能靠猜。

为进一步定位"skip 的是哪一种原因"（连不上 / 连上了但读不到数据字典），
又加了两个探针 step。理由同上：step 的 conclusion 是 jobs API 直接可读的，
而具体原因只写在 job log 里，会被 Oracle 容器的启动日志淹掉。
**当唯一可读的信道是红绿灯时，就把要问的问题拆成若干个是非题，
每题占一个 step。** 这条经验对任何"日志拿不到"的 CI 排查都适用。
