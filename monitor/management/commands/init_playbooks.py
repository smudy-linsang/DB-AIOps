...
PLAYBOOKS = [
    {
        ...
        'params_schema': {'blocker_id': {'required': True, 'type': 'session',
                                         'desc': '阻塞源会话ID；Oracle 使用 sid,serial#'}},
        'precheck': [{
            ...
            'sql_by_db': {
                ...
                'oracle': "SELECT 1 FROM v$session WHERE sid={blocker_id_sid}"},
    ...
    },
    {
        ...
        'params_schema': {'idle_sec': {'required': False, 'default': 600,
                                       'type': 'integer', 'min': 60, 'max': 86400,
                                       'desc': '空闲秒数阈值, 超过即清理'}},
        'precheck': [{'seq': 1, 'action': 'query', 'desc': '确认存在长空闲会话',
                      'sql': "SELECT id FROM information_schema.processlist WHERE command='Sleep' AND time>{idle_sec}",
                      'sql_by_db': {'pgsql': "SELECT pid FROM pg_stat_activity WHERE state='idle' AND state_change<now()-interval '{idle_sec} seconds'"},
                      'expect': 'rows>=1', 'on_fail': 'abort', 'timeout_sec': 10}],
        'steps': [{'seq': 1, 'action': 'execute', 'desc': '批量清理空闲超阈值会话',
                   'foreach_sql': "SELECT id FROM information_schema.processlist WHERE command='Sleep' AND time>{idle_sec}",
                   'foreach_sql_by_db': {'pgsql': "SELECT pid FROM pg_stat_activity WHERE state='idle' AND state_change<now()-interval '{idle_sec} seconds'"},
                   'sql': "KILL {item}",
    ...
    },
    {
        ...
        'params_schema': {
            ...
            'add_mb': {'required': False, 'default': 1024, 'type': 'integer',
                       'min': 64, 'max': 1048576}},
    ...
    },
    {
        ...
        'auto_execute': False, 'est_minutes': 3,
        'params_schema': {
            'param': {'required': True, 'type': 'identifier'},
            'old_value': {'required': True, 'type': 'literal'},
            'new_value': {'required': True, 'type': 'literal'}},
        ...
        'steps': [{'seq': 1, 'action': 'execute', 'desc': '回滚参数到原值',
                   ...
                   'expect': 'ok', 'on_fail': 'abort', 'timeout_sec': 15}],
        'verify': {'metric': '{param}', 'recover_expr': '== {old_value}', 'window_sec': 120,
                   'check_interval_sec': 10, 'min_stable_checks': 2, 'data_source': 'live_config'},
    ...
    },
    {
        ...
        'params_schema': {'session_id': {'required': True, 'type': 'session'}},
        ...
        'steps': [{'seq': 1, 'action': 'execute', 'desc': 'kill 慢SQL会话',
                   ...
                   'expect': 'ok', 'on_fail': 'abort', 'timeout_sec': 15}],
        'verify': {'metric': 'slow_queries_delta', 'recover_expr': '< 5', 'window_sec': 180,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for pb in PLAYBOOKS:
            values = dict(pb)
            playbook_id = values.pop('playbook_id')
            db_types = values.pop('applicable_db_types', [])
            precheck = values.pop('precheck', [])
            steps = values.pop('steps', [])
            rollback = values.pop('rollback', [])
            if options['force']:
                obj, created = Playbook.objects.update_or_create(
                    playbook_id=playbook_id, defaults=values)
            else:
                # 默认只补缺，不在每次部署时抹掉 DBA 已发布的定制内容。
                obj, created = Playbook.objects.get_or_create(
                    playbook_id=playbook_id, defaults=values)
            if created or options['force']:
                # 4NF 属性必须在父对象落库后写入；放进 defaults 会在模型构造
                # 阶段提前 save，随后 force_insert 用同一主键再次插入。
                obj.applicable_db_types = db_types
                obj.precheck = precheck
                obj.steps = steps
