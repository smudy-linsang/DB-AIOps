...
# 单次采集任务超时（秒）：超过此时间的采集视为失败，记�?DOWN，不阻塞其他�?COLLECT_TIMEOUT_SEC = getattr(settings, 'COLLECT_TIMEOUT_SEC', 15)
# 并发采集线程�?COLLECT_WORKERS = getattr(settings, 'COLLECT_WORKERS', 20)
...
class OracleChecker(BaseDBChecker):
    ...
    def collect_metrics(self, config, conn):
        ...
        # 5. QPS（基�?execute count / uptime�?        try:
        ...
        # 6. 慢查询统计（当前活跃且运行时间超�?10 秒的 SQL�?        try:
            cursor.execute("""
                SELECT count(*)
                FROM gv$session s
...
class PostgreSQLChecker(BaseDBChecker):
    ...
    def collect_metrics(self, config, conn):
        ...
        # 5. 数据库大小作为容量指标（PostgreSQL 表空间无独立配额限制�?        #    used_pct 无法�?SQL 层获取磁盘容量计算，此处只报告绝对大小，
...
class DamengChecker(BaseDBChecker):
    ...
    def get_connection(self, config):
        # 达梦 ODBC 连接字符�?        conn_str = f"DRIVER={{DM8 ODBC DRIVER}};SERVER={config.host}:{config.port};UID={config.username};PWD={config.get_password()};"
    ...
    def collect_metrics(self, config, conn):
        ...
        # 2. 连接�?        cur.execute("SELECT count(*) FROM v$sessions")
...
class GbaseChecker(BaseDBChecker):
    """Gbase8a 使用 MySQL 协议，复�?MySQLChecker 大部分逻辑"""
    ...
    def collect_metrics(self, config, conn):
        # Gbase8a �?MPP 架构，需要额外采集集群状�?        with conn.cursor() as cursor:
            # 1. 基础版本和连接数 (�?MySQL)
            cursor.execute("SELECT VERSION()")
            version = cursor.fetchone()['VERSION()']
            ...
            cluster_nodes = []
            try:
                # 尝试查询 Gbase 特有的集群视�?                cursor.execute("SELECT * FROM gcluster_v$node_status LIMIT 10")
                for row in cursor.fetchall():
...
class TDSQLChecker(BaseDBChecker):
    """
    TDSQL 有两种模式：MySQL 兼容�?�?PostgreSQL 兼容�?    这里默认�?MySQL 兼容版处理，如需 PG 版请切换�?PostgreSQLChecker
    """
    
    def get_connection(self, config):
        # TDSQL MySQL �?        return pymysql.connect(
            host=config.host, port=config.port, 
            user=config.username, password=config.get_password(),
            connect_timeout=5, cursorclass=pymysql.cursors.DictCursor
        )
    ...
    def collect_metrics(self, config, conn):
        # TDSQL 基本复用 MySQL 逻辑，增加分布式特性监�?        with conn.cursor() as cursor:
            # 1. 版本 (TDSQL 会有特殊标识)
            cursor.execute("SELECT VERSION()")
            version = cursor.fetchone()['VERSION()']
            ...
            shards_info = []
            try:
                # 尝试查询 TDSQL 特有的分片视�?                cursor.execute("SELECT * FROM tdsql_shard_status LIMIT 10")
                for row in cursor.fetchall():
                    shards_info.append({
...
class RedisChecker(BaseDBChecker):
    ...
    def check(self, config):
        ...
        self.cmd.process_result(config, 'DOWN', {
            "error": "Redis 监控尚未实现，需要安�?redis-py �?
        })
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print(f">> 架构模式：插件化检查器 + 基线异常检�?)
        print(f">> 告警策略：基线偏离告�?+ 锁等�?+ 容量监控")
        
        scheduler = BlockingScheduler()
        scheduler.add_job(self.monitor_job, 'interval', seconds=60)
        
        # 立即执行一�?        self.monitor_job() 
    ...
    def process_result(self, config, current_status, data):
        """统一结果处理和告警逻辑（v0.1.0：基�?AlertLog 去重�?""
        ...
        if current_status == 'UP':
            ...
            # A. 表空间容量告�?            tbs_warn = [t['name'] for t in data.get('tablespaces', [])
            ...
            # C. 锁等待告�?            current_locks = data.get('locks', [])
            am.fire_or_resolve(
            ...
            if current_locks:
                print(f"  🛑 [锁等待] {len(current_locks)} 个阻塞会�?)
    ...
    # 保留旧名称兼�?BaseDBChecker.check() 可能的外部调�?    def send_alert_email(self, config, title_prefix, error_msg):
